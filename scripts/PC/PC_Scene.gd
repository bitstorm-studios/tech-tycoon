...
func lauch_space_shooter():
	var space_shooter_instantiate = space_shooter_tutorial.instantiate()
...
func launch_catch_game():
	var catch_tutorial_instantiate = catch_tutorial.instantiate()
	get_tree().get_root().get_node("/root/Game").add_child(catch_tutorial_instantiate)
	get_node("/root/Game/HUD").hide()
